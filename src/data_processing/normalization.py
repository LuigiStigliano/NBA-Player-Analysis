"""
Questo modulo contiene le funzioni che ho usato per normalizzare le statistiche
e creare nuove metriche utili per l'analisi.
"""
...
def per_36_minutes_stats(df: DataFrame, stats_cols: list, minutes_played_col: str = "mp") -> DataFrame:
    """
    Ho normalizzato le statistiche su una base di 36 minuti per poter confrontare
    in modo equo giocatori con minutaggi diversi (titolari vs riserve).
    """
    ...
    for stat_col in stats_cols:
        if stat_col in df.columns:
            ...
            # La formula è semplice: (statistica / minuti giocati) * 36
...
def add_per_game_metrics(df: DataFrame) -> DataFrame:
    """
    Ho aggiunto anche le metriche 'per partita' (minuti e punti) perché
    sono utili per avere un contesto rapido sulle performance di un giocatore.
    """
