"""
Questo è lo script principale che esegue tutta la mia pipeline di analisi dei giocatori NBA.
L'ho progettata per essere eseguita dall'inizio alla fine, in modo automatizzato.
"""
import os
...
def run_pipeline():
    """
    Questa funzione esegue l'intera pipeline di analisi, passo dopo passo.
    """
    ...
    # Ho definito i percorsi qui per mantenere il codice pulito.
    ...
    # Prima di tutto, controllo se i dati grezzi esistono. Se no, li scarico.
    if not os.path.exists(raw_data_path):
        download_nba_dataset(raw_data_dir)
    
    # --- Fase 1: Ingestione e Pulizia ---
    print("\nFase 1: Carico, pulisco e preparo i dati")
    ...
    print("\nFase 2: Normalizzo le statistiche e creo nuove metriche.")
    ...
    # --- Fase 3: Clustering ---
    print("\nFase 3: Raggruppo i giocatori con il clustering.")

    # Qui ho dovuto gestire un caso particolare: i giocatori scambiati a metà stagione.
    # Per il clustering, volevo usare la stagione più recente e completa di ogni giocatore.
    ...
    # Do priorità alle righe 'TOT' che aggregano le statistiche di un giocatore in più squadre.
    ...
    # Queste sono le feature che ho scelto per definire lo stile di gioco.
    feature_cols = [
    ...
    # Dopo l'analisi esplorativa, ho visto che k=6 dava i cluster più interpretabili.
    optimal_k = 6
    print(f"Ho impostato k a {optimal_k} per ottenere cluster più significativi.")
    kmeans_model = train_kmeans_model(df_prepared, k=optimal_k)
    df_clustered = assign_clusters(kmeans_model, df_prepared)

    # Aggiorno la numerazione dei cluster da 0-5 a 1-6 per renderli più leggibili.
    df_clustered = df_clustered.withColumn("cluster_id", col("cluster_id") + 1)

    # --- Fase 4: Valutazione e Analisi ---
    print("\nFase 4: Analizzo i risultati.")
    ...
    print(f"\nFase 5: Salvo i risultati finali.")
    save_dataframe(df_final_analysis, final_output_path)
